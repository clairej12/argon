...
class EmbedEnvironment(Environment):
    ...
    def trajectory_reward(self, states, actions):
        pre_states, actions, post_states = (
            argon.tree.map(lambda x: x[:-1], states),
            argon.tree.map(lambda x: x[:-1], actions),
            argon.tree.map(lambda x: x[1:], states)
        )
        rewards = agt.vmap(self.reward)(
            pre_states, actions, post_states
        )
...
def create_data(rng_key, d, N, N_test, T, pair_first):
    ...
    def rollout(rng_key):
        ...
        rollout = argon.policy.rollout(
            agt.partial(env.step), x0, expert, 
...
def register_envs(registry : Registry, prefix=None):
    registry.register(
        ...
        partial(create_environment, rng_key=argon.random.key(42),
                d=4, pair_first=True),
        prefix=prefix
    )
    registry.register(
        "lower_bound/stable/2", 
        partial(create_environment, 
                rng_key=argon.random.key(42), 
                d=4, pair_first=False),
        prefix=prefix
    )
