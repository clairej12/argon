import argon.core.transforms as F
import argon.core.tree as tree
import argon.numpy as jnp

from argon.core.dataclasses import (
    dataclass, field, replace
)
from argon.core.typing import ArrayLike
    ...
    def __init__(self, tree: T | None = None):
        if tree is None:
            ...
            with jax.ensure_compile_time_eval():
                ns = jnp.array([(jnp.shape(x) + (1,))[0] for x in jax.tree_leaves(tree)], dtype=idx_dtype)
                if len(ns) > 0:
                    n = ns[0]
                    assert jnp.all(ns == n)
                else:
