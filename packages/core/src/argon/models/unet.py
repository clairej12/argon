...
import flax.linen.attention as activations
...
class ResBlock(nn.Module):
    ...
    def __call__(self, x, *, cond=None, train=False,
                             spatial_shape=None):
        skip_connection = nn.Conv(self.out_channels, self.spatial_dims*(1,))
        ...
        h = nn.Conv(self.out_channels,self.spatial_dims*self.kernel_size)(h)
        if cond is not None:
            film = nn.Sequential([
                nn.Dense(cond.shape[-1]),
                activations.silu,
            ...
            if self.use_scale_shift_norm:
                ...
                h = Normalization(self.spatial_dims)(h)
                h = h * (1 + scale) + shift
            else:
                h = Normalization(self.spatial_dims)(h + film)
        h = out_layers(h)
